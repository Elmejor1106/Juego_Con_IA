...
import { Share2, User, ArrowLeft, Eye, ChevronLeft, ChevronRight, X, Edit3 } from 'lucide-react';
...
const SharedGameScreen = () => {
  ...
  const loadSharedGame = async () => {
    ...
    try {
      ...
      // Si tiene permisos de edición, redirigir al editor después de un breve delay
      if (data.shareInfo.permissionLevel === 'edit') {
        setTimeout(() => {
          navigate(`/edit-game/${data.game.id}`, { 
            state: { 
              isCollaboration: true, 
              ownerName: data.shareInfo.ownerName 
            } 
          });
        }, 2000); // 2 segundos para mostrar mensaje de colaboración
      }
  ...
  // Mostrar mensaje especial para colaboraciones
  if (gameData?.shareInfo?.permissionLevel === 'edit') {
    return (
      <div className="w-screen h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center">
        <div className="text-center bg-white rounded-xl shadow-lg p-8 max-w-md mx-4">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">¡Colaboración Activada!</h2>
          <p className="text-gray-600 mb-4">
            <strong>{gameData.shareInfo.ownerName}</strong> te ha invitado a colaborar en este juego.
          </p>
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
            <p className="text-sm text-green-700">
              • El juego se ha agregado a "Mis Juegos"<br/>
              • Podrás editarlo cuando quieras<br/>
              • Redirigiendo al editor...
            </p>
          </div>
          <div className="flex items-center justify-center gap-2 text-green-600">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-green-600"></div>
            <span className="text-sm">Preparando editor...</span>
          </div>
        </div>
      </div>
    );
